...
from .models import (
    Task, Product, CartItem, Order, OrderItem,
    Genre, Movie, MovieReview, Tag, Post, Comment,
    Server, Book, BookRental, Course, StudentProfile, ExamResult
)
from .schemas import (
    MessageOut, TaskIn, TaskOut, ProductIn, ProductOut, CartItemOut, OrderOut,
    GenreIn, MovieIn, MovieReviewIn, MovieOut, PostIn, CommentIn, PostOut,
    ServerIn, ServerMetricIn, ServerOut, BookIn, BookOut, CourseIn, CourseOut,
    StudentIn, ExamResultIn, CourseStatsOut
)
...
def get_tasks(request, status: Optional[bool] = None, order_by: Optional[str] = "created_at"):
...
def checkout_order(request):
    ...
    return {
        ...
        "items": [{"product_name": i.product.name if i.product else "Видалений товар", "price": float(i.price), "quantity": i.quantity} for i in order.items.all()]
    }

movie_router = Router(auth=django_auth)

@movie_router.get("/", response=list[MovieOut])
def get_movies(request, genre: Optional[str] = None, rating: Optional[float] = None, search: Optional[str] = None):
    ...
    return [{
        ...
        "release_date": m.release_date, "rating": float(m.rating),
...
def create_movie(request, payload: MovieIn):
    movie = Movie.objects.create(
        title=payload.title, description=payload.description, release_date=payload.release_date
    )
...
def add_movie_review(request, movie_id: int, payload: MovieReviewIn):
    ...
    MovieReview.objects.create(movie=movie, user=request.auth, text=payload.text, rating=payload.rating)
...
def create_post(request, payload: PostIn):
    post = Post.objects.create(author=request.auth, title=payload.title, content=payload.content)
...
def update_server_metrics(request, server_id: int, payload: ServerMetricIn):
    ...
    if server.cpu_usage > 90.0 or server.ram_usage > 90.0:
        logger.warning(f"[CRITICAL ALERT] Сервер {server.name} ({server.ip_address}) перевищив ліміти! CPU: {server.cpu_usage}%, RAM: {server.ram_usage}%")
...
def search_books(request, search_query: Optional[str] = None):
    ...
    if search_query:
        queryset = queryset.filter(title__icontains=search_query) | queryset.filter(author__icontains=search_query) | queryset.filter(genre__icontains=search_query)
    return queryset
...
def enroll_student(request, course_id: int):
    ...
    profile, _ = StudentProfile.objects.get_or_create(user=request.auth, defaults={"full_name": request.auth.username})
...
def get_course_stats(request, course_id: int):
    ...
    course = get_object_or_404(Course, id=course_id)
    stats = ExamResult.objects.filter(course=course).aggregate(avg_grade=Avg('grade'), total=Count('id'))
